...
- 新保存的灵感默认仅自己可见；明确勾选公开后，提示词、生成参数及素材才会分享给其他已登录用户，作者可以撤回。旧私人分享不会自动公开。
- 公开素材独立保存；新外链必须转成上传素材（浏览器会尝试读取转换），失效或无法读取的链接可以在分享弹窗里重新上传。删除旧历史前，会先保全旧私人分享仍在引用的素材。
- 主提示词字段支持最多 100,000 个 JavaScript 字符；超限明确提示，不再静默截断。上游模型自己的请求限制仍然适用。
- 个人备份不包含全站公开分享，恢复不会重新发布作品。恢复使用日志及整体回滚，支持异常与进程中断后恢复；不承诺 fsync/断电级事务，文件后端仍要求单写进程。
